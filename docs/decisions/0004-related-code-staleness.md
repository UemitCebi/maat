...
CodeDoc is language-agnostic and cannot parse every codebase to know whether a
doc is *semantically* current.

## Decision

Each doc may declare a `related_code` list in its front-matter naming the
source paths it describes. `codedoc check` compares modification times: if any
...
promoted to an error via `.codedoc.yml` or the `--strict` flag. Two adjacent
...
- **Git-based diffing** in CI (did the PR touch code under a doc's paths without
  ...
  also works for a local `codedoc check` on a plain directory.
